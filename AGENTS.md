...
- Keep each collection README brief and stable. Link to the published
  GitHub Pages docs generated by `antsibull-docs` for all detailed
  content (full plugin lists, options, examples, change history).
- If a topic is covered in the generated docs, do not duplicate it in
  the README. Prefer a short overview, install snippet, and a docs link.
  Let antsibull handle the heavy lifting and frequent content changes.
